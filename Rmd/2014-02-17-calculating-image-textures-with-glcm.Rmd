...
```{r, mean, fig.cap='"mean" GLCM texture calculated by glcm R package'}
plot(textures$glcm_mean)
```

```{r, variance, fig.cap='"variance" GLCM texture calculated by glcm R package'}
plot(textures$glcm_variance)
```

```{r, homogeneity, fig.cap='"homogeneity" GLCM texture calculated by glcm R package'}
plot(textures$glcm_homogeneity)
```

```{r, contrast, fig.cap='"contrast" GLCM texture calculated by glcm R package'}
plot(textures$glcm_contrast)
```

```{r, dissimilarity, fig.cap='"dissimilarity" GLCM texture calculated by glcm R package'}
plot(textures$glcm_dissimilarity)
```

```{r, entropy, fig.cap='"entropy" GLCM texture calculated by glcm R package'}
plot(textures$glcm_entropy)
```

```{r, second_moment, fig.cap='"second moment" GLCM texture calculated by glcm R package'}
plot(textures$glcm_second_moment)
```

```{r, correlation, fig.cap='"correlation" GLCM texture calculated by glcm R package'}
