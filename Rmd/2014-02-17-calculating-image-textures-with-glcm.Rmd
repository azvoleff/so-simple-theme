...
```{r, fig.cap='"mean" GLCM texture calculated by glcm R package'}
plot(textures$glcm_mean)
```

```{r, fig.cap='"variance" GLCM texture calculated by glcm R package'}
plot(textures$glcm_variance)
```

```{r, fig.cap='"homogeneity" GLCM texture calculated by glcm R package'}
plot(textures$glcm_homogeneity)
```

```{r, fig.cap='"contrast" GLCM texture calculated by glcm R package'}
plot(textures$glcm_contrast)
```

```{r, fig.cap='"dissimilarity" GLCM texture calculated by glcm R package'}
plot(textures$glcm_dissimilarity)
```

```{r, fig.cap='"entropy" GLCM texture calculated by glcm R package'}
plot(textures$glcm_entropy)
```

```{r, fig.cap='"second moment" GLCM texture calculated by glcm R package'}
plot(textures$glcm_second_moment)
```

```{r, fig.cap='"correlation" GLCM texture calculated by glcm R package'}
