...
# plotRGB(base_filled, stretch="hist")
```
...
plotRGB(base_filled_fast, stretch="hist")
```
...
`cloud_remove` with `use_IDL=FALSE`:
...
plotRGB(base_filled_R, stretch="hist")
```
...
evaluation <- stack(base_filled[[3]],
                    base_filled_fast[[3]],
                    base_filled_R[[3]])
names(evaluation) <- c("CLOUD_REMOVE", "CLOUD_REMOVE_FAST", "use_IDL_FALSE")
plot(evaluation, nc=3)
```
